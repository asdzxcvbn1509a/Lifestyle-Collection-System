    ...
    <div className="card card-hover group flex flex-col overflow-hidden">
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        ...
        <div className="mt-2 flex items-center gap-1 border-t border-slate-100 pt-2 dark:border-slate-800">
          <button onClick={() => onEdit(item)} className="btn-ghost flex-1 py-1 text-xs">
            <Pencil className="h-3.5 w-3.5" /> แก้ไข
          </button>
          <button
            onClick={() => onDelete(item)}
            className="btn-ghost flex-1 py-1 text-xs text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/10"
          >
